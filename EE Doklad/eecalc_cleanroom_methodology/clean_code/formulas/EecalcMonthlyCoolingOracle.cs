...
namespace EE.Doklad.Tests.Validation
{
    ...
    public sealed class EecalcMonthlyCoolingOracle
    {
        ...
        private readonly CoolingFreeMode coolingFreeMode;

        public EecalcMonthlyCoolingOracle(CoolingFreeMode coolingFreeMode = CoolingFreeMode.EECalcLegacy)
        {
            this.coolingFreeMode = coolingFreeMode;
        }
        ...
        public double QfreeCooling(EecalcValidationFixture fixture, EecalcMonthlyDaysOracleRow month)
        {
            ...
            var work = SumNightFreeCooling(weather, fixture.NightVentilationWorkdaySchedule, hfree, fixture.ProjectTemperature, coolingFreeMode)
                * month.WorkDays;
            var saturday = SumNightFreeCooling(weather, fixture.NightVentilationSaturdaySchedule, hfree, fixture.ProjectTemperature, coolingFreeMode)
                * month.Saturdays;
            var sunday = SumNightFreeCooling(weather, fixture.NightVentilationSundaySchedule, hfree, fixture.ProjectTemperature, coolingFreeMode)
                * month.Sundays;

            // KD-C006: EECalc uses the Sunday night-ventilation schedule for holidays.
            var holiday = SumNightFreeCooling(weather, fixture.NightVentilationSundaySchedule, hfree, fixture.NonProjectTemperature, coolingFreeMode)
                * month.Holidays;
        ...
        public double QveCooling(EecalcValidationFixture fixture, EecalcMonthlyDaysOracleRow month)
        {
            ...
            var work = SumQveDay(
                ...
                inclusiveCoolingScheduleEnd: false) * month.WorkDays;
            var saturday = SumQveDay(
                ...
                inclusiveCoolingScheduleEnd: true) * month.Saturdays;
            var sunday = SumQveDay(
                ...
                inclusiveCoolingScheduleEnd: true) * month.Sundays;

            return work + saturday + sunday;
        }
        ...
        private static double SumNightFreeCooling(
            ...
            CoolingFreeMode mode)
        {
            return GetNightWorkingHours(schedule.StartHour, schedule.EndHour)
                .Sum(hour =>
                {
                    var delta = indoorTemperature - WeatherAt(weather, hour).Temperature;
                    if (mode == CoolingFreeMode.RegulationStrict)
                    {
                        delta = Math.Max(0.0, delta);
                    }

                    return hfree * delta / 1000.0;
                });
        }
        ...
        private static double SumQveDay(
            ...
            double nonProjectTemperature,
            EecalcDailySchedule ventilation,
            EecalcDailySchedule cooling,
            bool inclusiveCoolingScheduleEnd)
        {
            var result = 0.0;

            for (var hour = ventilation.StartHour; hour < ventilation.EndHour && hour < 24; hour++)
            {
                var indoor = IsCoolingProjectHour(hour, cooling, inclusiveCoolingScheduleEnd)
                    ? projectTemperature
                    : nonProjectTemperature;
                result += hve * (indoor - flowTemperature) / 1000.0;
            }
        ...
        private static bool IsCoolingProjectHour(
            ...
            bool inclusiveCoolingScheduleEnd)
        {
            return hour >= cooling.StartHour
                && (inclusiveCoolingScheduleEnd ? hour <= cooling.EndHour : hour < cooling.EndHour);
        }
