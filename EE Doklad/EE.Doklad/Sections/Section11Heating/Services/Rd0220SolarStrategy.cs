...
public class Rd0220SolarStrategy : ISolarStrategy
{
        ...
        int monthIndex)
    {
        ...
        foreach (var window in windows)
        {
            ...
            double asol = window.Fw * window.G * (1.0 - window.Ffr) * window.Area;
            double hr = CalcHr(window.Epsilon);
            double phiR = HeatingConstants.Rse * window.U * window.Area * hr * HeatingConstants.DeltaTheta;
            double phiSol = asol * isol - HeatingConstants.Fsky_Vertical * phiR;
            qgn += phiSol * dt / 1000.0;
        }
