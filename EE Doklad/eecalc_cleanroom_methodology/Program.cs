...
var coolingVentilationHumidity = GetOptionDouble(args, "--cooling-vent-rh", 40.0, allowNegative: false);
...
var coolingCoreFlowHumidity = GetOptionDouble(args, "--cooling-core-flow-rh", 0.0, allowNegative: false);
...
var coolingFreeMode = GetCoolingFreeMode(args);
var coolingWorkSchedule = GetSchedule(args, "--cooling-work", 0, 24);
var coolingSaturdaySchedule = GetSchedule(args, "--cooling-sat", 0, 24);
var coolingSundaySchedule = GetSchedule(args, "--cooling-sun", 0, 24);
var coolingOccupantsWorkSchedule = GetSchedule(args, "--cooling-occupants-work", 0, 12);
var coolingOccupantsSaturdaySchedule = GetSchedule(args, "--cooling-occupants-sat", 0, 12);
var coolingOccupantsSundaySchedule = GetSchedule(args, "--cooling-occupants-sun", 0, 12);
var coolingVentilationWorkSchedule = GetSchedule(args, "--cooling-vent-work", 0, 20);
var coolingVentilationSaturdaySchedule = GetSchedule(args, "--cooling-vent-sat", 0, 20);
var coolingVentilationSundaySchedule = GetSchedule(args, "--cooling-vent-sun", 0, 20);
...
var coolingCalculation = CopyCalculationWithClimate(
    ...
    workdaySchedule: coolingWorkSchedule,
    saturdaySchedule: coolingSaturdaySchedule,
    sundaySchedule: coolingSundaySchedule,
    occupantsWorkdaySchedule: coolingOccupantsWorkSchedule,
    occupantsSaturdaySchedule: coolingOccupantsSaturdaySchedule,
    occupantsSundaySchedule: coolingOccupantsSundaySchedule,
    ventilationWorkdaySchedule: coolingVentilationWorkSchedule,
    ventilationSaturdaySchedule: coolingVentilationSaturdaySchedule,
    ventilationSundaySchedule: coolingVentilationSundaySchedule,
...
static CoolingFreeMode GetCoolingFreeMode(string[] args)
{
    var optionIndex = Array.IndexOf(args, "--cooling-free-mode");
    if (optionIndex < 0)
    {
        return CoolingFreeMode.EECalcLegacy;
    }

    if (optionIndex == args.Length - 1)
    {
        throw new ArgumentException("--cooling-free-mode expects legacy or regulation.");
    }

    return args[optionIndex + 1].ToLowerInvariant() switch
    {
        "legacy" => CoolingFreeMode.EECalcLegacy,
        "regulation" => CoolingFreeMode.RegulationStrict,
        _ => throw new ArgumentException("--cooling-free-mode expects legacy or regulation.")
    };
}
...
{
    RunComponentSavingsVerification(fixture, coolingFixture, calculation, coolingCalculation, coolingEsmFlowTemperature, coolingFreeMode);
    return;
}

if (args.Contains("--verify-full-project"))
{
    RunFullProjectVerification(fixture, coolingFixture, calculation, coolingCalculation, dhwCalculation, coolingFlowTemperature, coolingEsmFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity, coolingEsmFreeDebit, coolingFreeMode);
    return;
}

if (args.Contains("--verify-cooling-details"))
{
    RunCoolingDetailsVerification(coolingFixture, coolingCalculation, coolingFlowTemperature, coolingEsmFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity, coolingEsmFreeDebit, coolingFreeMode);
    return;
}
...
var coolingVentInput = BuildCoolingVentilationFor(coolingCalculation, coolingFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
...
var cooling = new EecalcMonthlyCoolingOracle(coolingFreeMode).Calculate(coolingFixture, coolingVentilationInputs);
...
static string CoolingFreeModeLabel(CoolingFreeMode mode)
{
    return mode switch
    {
        CoolingFreeMode.EECalcLegacy => "legacy",
        CoolingFreeMode.RegulationStrict => "regulation",
        _ => mode.ToString()
    };
}
...
static void RunFullProjectVerification(
    ...
    double coolingVentilationHumidity,
    double coolingEsmFreeDebit,
    CoolingFreeMode coolingFreeMode)
{
    ...
    var coolingVentilationSource = BuildCoolingVentilationFor(coolingCalculation, coolingFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
    var coolingVentilationEsmSource = BuildCoolingVentilationFor(esmCoolingCalculation, coolingEsmFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
    ...
    var coolingCoreInput = BuildCoolingVentilationFor(coolingCalculation, coolingFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
    var esmCoolingCoreInput = BuildCoolingVentilationFor(esmCoolingCalculation, coolingEsmFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
    ...
    var currentCooling = CalculateCoolingNeededSource(currentCoolingFixture, coolingCoreInput, coolingFreeMode);
    var esmCooling = CalculateCoolingNeededSource(esmCoolingFixture, esmCoolingCoreInput, coolingFreeMode);
    ...
    CoolingFreeMode coolingFreeMode)
{
    ...
    var coolingVentilation = BuildCoolingVentilationFor(coolingCalculation, coolingFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
    ...
    var esmCoolingVentilation = BuildCoolingVentilationFor(esm.Calculation, coolingEsmFlowTemperature, coolingVentilationDebit, coolingVentilationHumidity);
    var baseState = CalculateCoolingState(baseLine, coolingVentilation, coolingEfficiency, coolingFreeMode);
    var esmEnvelopeOnlyState = CalculateCoolingState(esmEnvelopeOnly, coolingVentilation, coolingEfficiency, coolingFreeMode);
    var esmState = CalculateCoolingState(esm, esmCoolingVentilation, coolingEfficiency, coolingFreeMode);
    var savings = new EecalcEnvelopeSavingsOracle().Calculate(
        baseLine,
        esm,
        fixture => CalculateCoolingState(fixture, coolingVentilation, coolingEfficiency, coolingFreeMode).NeededEnergy);

    Console.WriteLine($"Cooling diagnostics. Climate zone {coolingCalculation.ClimateZoneId}.");
    Console.WriteLine($"Cooling free mode: {CoolingFreeModeLabel(coolingFreeMode)}.");
    Console.WriteLine($"Core cooling flow temperature: {Format(coolingCalculation.FlowTemperature)}.");
    Console.WriteLine($"Core cooling flow relative humidity: {Format(coolingCalculation.FlowRelativeHumidity)}.");
    ...
    Console.WriteLine($"Ventilation cooling relative humidity: {Format(coolingVentilationHumidity)}.");
    ...
    PrintCoolingRawDetails("Current", baseLine, coolingFreeMode);
    PrintCoolingRawDetails("ESM", esm, coolingFreeMode);
    PrintFreeCoolingDetails(baseLine, esm, coolingFreeMode);
    PrintCoolingFreeModeComparison(baseLine, esm);
    PrintCoolingExactReconciliation(baseLine, esm, coolingVentilation, esmCoolingVentilation, coolingEfficiency, coolingFreeMode);
    ...
    CoolingFreeMode coolingFreeMode)
{
    ...
    var coolingOracle = new EecalcMonthlyCoolingOracle(coolingFreeMode);
    var baseCoolingClean = coolingOracle.Calculate(baseLine, baseCleanVentilationInputs);
    var esmCoolingClean = coolingOracle.Calculate(esm, esmCleanVentilationInputs);
    var baseCoolingDecompiledVent = coolingOracle.Calculate(baseLine, baseDecompiledVentilationInputs);
    var esmCoolingDecompiledVent = coolingOracle.Calculate(esm, esmDecompiledVentilationInputs);
...
{
    var coolingOracle = new EecalcMonthlyCoolingOracle(coolingFreeMode);
    var currentCooling = coolingOracle.Calculate(current);
    var esmCooling = coolingOracle.Calculate(esm);

    Console.WriteLine();
    Console.WriteLine($"No-treatment/free-cooling inputs by month, kWh/m2 ({CoolingFreeModeLabel(coolingFreeMode)})");
...
static void PrintCoolingFreeModeComparison(
    EecalcEnvelopeFixture current,
    EecalcEnvelopeFixture esm)
{
    var currentLegacy = new EecalcMonthlyCoolingOracle(CoolingFreeMode.EECalcLegacy).Calculate(current);
    var currentRegulation = new EecalcMonthlyCoolingOracle(CoolingFreeMode.RegulationStrict).Calculate(current);
    var esmLegacy = new EecalcMonthlyCoolingOracle(CoolingFreeMode.EECalcLegacy).Calculate(esm);
    var esmRegulation = new EecalcMonthlyCoolingOracle(CoolingFreeMode.RegulationStrict).Calculate(esm);

    Console.WriteLine();
    Console.WriteLine("No-treatment/free-cooling mode comparison, kWh/m2");
    Console.WriteLine("State | Legacy | Regulation | Diff regulation-legacy");
    Console.WriteLine("------|--------|------------|-----------------------");
    PrintCoolingFreeModeComparisonRow("Current", currentLegacy.ResultCoolingInputs, currentRegulation.ResultCoolingInputs);
    PrintCoolingFreeModeComparisonRow("ESM", esmLegacy.ResultCoolingInputs, esmRegulation.ResultCoolingInputs);
}

static void PrintCoolingFreeModeComparisonRow(
    string state,
    double legacy,
    double regulation)
{
    Console.WriteLine(string.Join(
        ...
        Format6(legacy),
        Format6(regulation),
        Format6(regulation - legacy)));
}

static void PrintCoolingRawDetails(string state, EecalcEnvelopeFixture fixture, CoolingFreeMode coolingFreeMode)
{
    var cooling = new EecalcMonthlyCoolingOracle(coolingFreeMode).Calculate(fixture);

    Console.WriteLine();
    Console.WriteLine($"Cooling raw details by month, kWh before area normalization ({state}, {CoolingFreeModeLabel(coolingFreeMode)})");
    Console.WriteLine("Month | Qgain | Qtr | Qinf | Qloss | Ac | Gamma | Eta | LatOcc | LatInf | LatVent | QcoolRaw | Qfree");
    Console.WriteLine("------|-------|-----|------|-------|----|-------|-----|--------|--------|---------|----------|------");
    foreach (var row in cooling.Rows)
    {
        Console.WriteLine(string.Join(
    ...
    Console.WriteLine(string.Join(
        ...
        string.Empty,
        string.Empty,
        Format6(cooling.Rows.Sum(row => row.QLatentOccupants) / fixture.Calculation.HeatedArea),
        Format6(cooling.Rows.Sum(row => row.QLatentInf) / fixture.Calculation.HeatedArea),
        Format6(cooling.Rows.Sum(row => row.QLatentVent) / fixture.Calculation.HeatedArea),
        Format6(cooling.ResultNoInputsNetEnergy),
        Format6(cooling.ResultCoolingInputs)));
}
        ...
        CoolingFreeMode coolingFreeMode)
{
    ...
    var cooling = new EecalcMonthlyCoolingOracle(coolingFreeMode).Calculate(fixture, ventilationInputs);
...
static EECalcVentilationInput BuildCoolingVentilationFor(
    ...
    double debit = 0.500,
    double flowRelativeHumidity = 40.0)
{
    return EecalcVentilationEsmCalculator.Clone(
        TestFixture.BuildCoolingVentilation(),
        debit: debit,
        flowTemperature: flowTemperature,
        flowRelativeHumidity: flowRelativeHumidity,
        workdaySchedule: coolingCalculation.VentilationWorkdaySchedule,
        saturdaySchedule: coolingCalculation.VentilationSaturdaySchedule,
        sundaySchedule: coolingCalculation.VentilationSundaySchedule);
}
    ...
    CoolingFreeMode coolingFreeMode)
{
    var ventilation = new EECalcVentilationOracle().Calculate(fixture.Calculation, coolingVentilationInput);
    var ventilationInputs = ventilation.Rows.Sum(row => row.CoolingInputs);
    return new EecalcMonthlyCoolingOracle(coolingFreeMode).Calculate(fixture, ventilationInputs).ResultNetEnergy;
}
...
static double CalculateCoolingNeededSource(
    ...
    CoolingFreeMode coolingFreeMode)
{
    var netEnergy = CalculateCoolingNeeded(fixture, coolingVentilationInput, coolingFreeMode);
...
static void RunComponentSavingsVerification(
    ...
    CoolingFreeMode coolingFreeMode)
{
    var coolingCalculator = new EecalcCoolingEsmCalculator(coolingFreeMode);
    var esmCoolingCalculation = CloneCalculationWithCoolingLoads(
        coolingCalculation,
        lightsCoolingPower: 0.40,
        lightsCoolingWorkSchedule: 40.0);
