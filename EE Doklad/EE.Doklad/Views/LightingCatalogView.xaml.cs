...
{
    public partial class LightingCatalogView : UserControl
    {
        public LightingCatalogView()
        {
            InitializeComponent();
            Loaded += LightingCatalogView_Loaded;
        }

        private void LightingCatalogView_Loaded(object? sender, System.Windows.RoutedEventArgs e)
        {
        ...
        {
            if (DataContext is not LightingCatalogViewModel vm) return;

            var dlg = new SaveFileDialog
            {
                Title = "Експортиране на потребителски осветителни компоненти",
                Filter = "JSON files (*.json)|*.json|All files (*.*)|*.*",
                DefaultExt = "json",
                FileName = "lighting.user.export.json"
            };
        ...
        private void ImportButton_Click(object? sender, System.Windows.RoutedEventArgs e)
        {
            ...
            {
                Title = "Импортиране на потребителски осветителни компоненти",
                Filter = "JSON files (*.json)|*.json|All files (*.*)|*.*",
                DefaultExt = "json"
            };
            ...
            try
            {
                var imported = JsonConvert.DeserializeObject<List<LightingUser>>(json);
        ...
        private void DataGrid_RowEditEnding(object sender, DataGridRowEditEndingEventArgs e)
        {
            ...
            if (e.Row.Item is not LightingRow row) return;
